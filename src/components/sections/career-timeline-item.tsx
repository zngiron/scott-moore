...
import { careerItems } from '@/data/static/career';
...
function TimelineItem({
  ...
  const bulletScale = useTransform(scrollProgress, [itemStart, itemStart + 0.01], [0, 1]);
  const bulletOpacity = useTransform(scrollProgress, [itemStart, itemStart + 0.01], [0, 1]);
  const yearOpacity = useTransform(scrollProgress, [itemStart, itemStart + 0.02], [0, 1]);
  const yearX = useTransform(scrollProgress, [itemStart, itemStart + 0.02], [-10, 0]);
  ...
  return (
    <div className="flex w-full flex-col">
      ...
      <div className="flex gap-4">
        ...
        >
          <p className="font-display text-lg leading-6 text-white">{item.position}</p>
...
export function CareerContent() {
  ...
  const reversedItems = [...careerItems].reverse();
    ...
    >
      <div className="sticky top-0 flex h-dvh items-center">
        <div className={cn('mx-auto w-full max-w-screen-2xl px-6', 'lg:px-36')}>
          <div className={cn('grid grid-cols-1 gap-12', 'lg:grid-cols-2 lg:gap-24')}>
            <div className="flex flex-col gap-6">
              <p className="text-xl uppercase tracking-widest text-white/60">Career</p>
              <h2 className={cn('font-display text-4xl font-light leading-tight tracking-tight', 'md:text-6xl')}>
